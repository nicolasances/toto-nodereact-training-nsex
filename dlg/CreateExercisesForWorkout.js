...
exports.do = function(correlationId, sessionId, workout) {
  ...
  logger.compute(correlationId, 'Retrieving last session for workout ' + JSON.stringify(workout));
  ...
  return new Promise((success, failure) => {
    ...
    getLastSessionWithWorkout.do(correlationId, workout).then((data) {
      ...
      logger.compute(correlationId, 'Last session for workout: ' + JSON.stringify(data));
      ...
      if (lastSessionId != null) {
        ...
        logger.compute(correlationId, 'Retrieving previous session exercises.');

        // 2.a.1. Retrieve the exercises GET /session/:id/exercises
        getSessionExercises.do(correlationId, lastSessionId).then((exercises) => {

          logger.compute(correlationId, 'Found exercises, POSTing them for the new session.');
